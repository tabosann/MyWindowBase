...
int main()
{
	...
	WNDCLASSEX wndClass = hwnd.GetWndClass();					// 元のウィンドウ設定を取得
...
LRESULT GameWindow::LocalWndProc(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam)
{
	...
	case WM_PAINT:
	{
		...
		TCHAR str[256];
		int length = _stprintf_s(str, _T("gGameCounter : %d"), gGameCounter);
		TextOut(hdc, drawPos.x, drawPos.y, str, length);

		drawPos.y += metric.tmHeight; //改行

		length = _stprintf_s(str, _T("gMsgCounter : %d"), gMsgCounter);
		TextOut(hdc, drawPos.x, drawPos.y, str, length);

		drawPos.y += metric.tmHeight; //改行

		length = _stprintf_s(str, _T("gPaintCounter : %d"), gPaintCounter);
		TextOut(hdc, drawPos.x, drawPos.y, str, length);
